...
class As {
  ...
  public static arrayOf<T>(
    ...
    if (!Is.arrayOf<T>(value, innerTester)) {
      throw new AssertionError({
        message: `${value} is not an array of specified type`,
      });
    }
  }
  ...
  public static readonlyArrayOf<T>(
    ...
    if (!Is.readonlyArrayOf<T>(value, innerTester)) {
      throw new AssertionError({
        message: `${value} is not a readonly array of specified type`,
      });
    }
  }
   ...
   * @param value The value to test
   * @deprecated This isn't correct so shouldn't be used
   */
